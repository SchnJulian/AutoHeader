...
void MainWindow::initExtensions() {
  ...
  extensions.insert(std::make_pair("XX", 5));
...
QString MainWindow::commentHeader(const QString& header,
  ...
  return ((header.count("\n") >= 1 && language.getMultiLine().length() == 2)
              ? commentHeaderMultiLine(language)
              : commentHeaderSingleLine(language));
}
QString MainWindow::commentHeaderSingleLine(
    const Language::Language& language) {
  QString temp = ui->fileEditor->toPlainText(), line, final;
...
QString MainWindow::commentHeaderMultiLine(const Language::Language& language) {
  QString temp = ui->fileEditor->toPlainText();
...
void MainWindow::fillFileList(QStringList files) {
...
void MainWindow::processFiles() {
  ...
  for (int i = 0; i < fileNameList.length(); i++) {
    ...
    if (l.first) {
      ...
      if (header.autoDetect) {
        out << commentHeader(header.getContent(), l.second) << "\n\n" << buffer;
      } else {
        out << commentHeader(header.getContent(), language) << "\n\n" << buffer;
      }
...
/// the new file.
///
void MainWindow::processFilesSafeMode() {
  QString temp, tempOut, dirName;
  std::pair<bool, Language::Language> l;
  for (int i = 0; i < fileNameList.length(); i++) {
    temp = currentDirectory.absolutePath() + "/" + fileNameList[i];
    QFileInfo fi(temp);
...
void MainWindow::on_commentStyleComboBox_activated(int index) {
  selectLanguageFromIndex(index);
}
void MainWindow::selectLanguageFromIndex(int index) {
  (index == 0) ? header.autoDetect = true : header.autoDetect = false;
